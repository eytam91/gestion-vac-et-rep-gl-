...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-stone-900 text-white p-3.5 rounded-xl shadow-xl text-xs space-y-2 border border-stone-700 min-w-[210px]">
          <div className="border-b border-stone-800 pb-1.5 flex justify-between items-center gap-2">
            <span className="font-bold text-sm text-amber-300">{data.name}</span>
            <span className="text-3xs px-2 py-0.5 rounded bg-stone-800 text-stone-300 font-mono">
              {data.contractType}
            </span>
          </div>
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-emerald-400">+{data.acquis} j</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-stone-400">Congés Pris :</span>
              <span className="font-bold text-amber-400">-{data.pris} j</span>
            </div>
    ...
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-stone-700/60 border border-stone-600/60 text-amber-300 text-xs font-semibold uppercase tracking-wider">
              ...
              Centre d'Administration & Suivi des Congés
            </div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white">
              Gestion du Solde de Congés & Absences
            </h1>
            <p className="text-stone-300 text-sm max-w-2xl leading-relaxed">
              Enregistrez les congés passés et actuels pour calculer automatiquement le solde réel. Recevez une alerte immédiate lorsqu'un employé a pris plus de jours qu'il n'en a acquis.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            >
              ...
              Saisir Congé Passé / Antérieur
            </button>
            <button
              id="btn-dashboard-new-leave"
              onClick={() => onOpenLeaveModal()}
              className="inline-flex items-center justify-center gap-2 bg-amber-400 hover:bg-amber-300 text-stone-950 font-bold text-sm px-5 py-3 rounded-xl transition-all shadow-md active:scale-98 cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              Saisir Nouveaux Congés
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-stone-200/80 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider">Effectif Total</p>
            <p className="text-2xl font-bold text-stone-900 mt-1">{totalEmployees}</p>
            <p className="text-xs text-stone-500 mt-0.5">
              {statsList.filter((s) => s.employee.contractType === 'TYPE_A').length} Type A (6-Mois) | {statsList.filter((s) => s.employee.contractType === 'TYPE_B').length} Type B
            </p>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-stone-200/80 shadow-xs flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">
              Avances de congés à résorber par l'activité
            </p>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-stone-200/80 shadow-xs flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">
              Total consommé sur l'ensemble du personnel
            </p>
          </div>
      ...
      {/* Summary of Authorized Absences */}
      <div className="bg-white rounded-2xl border border-stone-200/80 shadow-xs p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-stone-100 pb-3">
          <h3 className="text-base font-bold text-stone-900 flex items-center gap-2">
            <HeartPulse className="w-5 h-5 text-stone-700" />
            Récapitulatif des Absences Spécifiques Autorisées
          </h3>
          <span className="text-xs font-semibold text-stone-400">Total hors Congés Payés</span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <div className="bg-red-50/70 border border-red-200/60 p-3.5 rounded-xl">
            <p className="text-xs font-bold text-red-900">{LEAVE_TYPE_LABELS.MALADIE_JUSTIFIEE}</p>
            <p className="text-xl font-bold text-red-950 mt-1">{globalLeaves.maladie} <span className="text-xs font-normal">jours</span></p>
          </div>

          <div className="bg-blue-50/70 border border-blue-200/60 p-3.5 rounded-xl">
            <p className="text-xs font-bold text-blue-900">{LEAVE_TYPE_LABELS.PATERNITE}</p>
            <p className="text-xl font-bold text-blue-950 mt-1">{globalLeaves.paternite} <span className="text-xs font-normal">jours</span></p>
          </div>

          <div className="bg-purple-50/70 border border-purple-200/60 p-3.5 rounded-xl">
            <p className="text-xs font-bold text-purple-900">{LEAVE_TYPE_LABELS.MARIAGE}</p>
            <p className="text-xl font-bold text-purple-950 mt-1">{globalLeaves.mariage} <span className="text-xs font-normal">jours</span></p>
          </div>

          <div className="bg-stone-100 border border-stone-200 p-3.5 rounded-xl">
            <p className="text-xs font-bold text-stone-800">{LEAVE_TYPE_LABELS.DECES}</p>
            <p className="text-xl font-bold text-stone-900 mt-1">{globalLeaves.deces} <span className="text-xs font-normal">jours</span></p>
          </div>

          <div className="bg-emerald-50/70 border border-emerald-200/60 p-3.5 rounded-xl col-span-2 sm:col-span-1">
            <p className="text-xs font-bold text-emerald-900">{LEAVE_TYPE_LABELS.AUTRE}</p>
            <p className="text-xl font-bold text-emerald-950 mt-1">{globalLeaves.autre} <span className="text-xs font-normal">jours</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-stone-200/80 shadow-xs overflow-hidden">
        <div className="p-5 border-b border-stone-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-bold text-stone-900">Tableau Général des Soldes & Régularisations de Congés</h3>
            <p className="text-xs text-stone-500 mt-0.5">
              Visualisation du solde de congés actuel, de la consommation et du temps de travail nécessaire pour résorber un solde négatif.
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-stone-50/80 border-b border-stone-200/80 text-stone-500 text-xs font-semibold uppercase tracking-wider">
                <th className="py-3.5 px-6">Employé</th>
              ...
              {statsList.map(({ employee: emp, stats }) => (
                <tr key={emp.id} className="hover:bg-stone-50/60 transition-colors">
                  ...
                  <td className="py-4 px-6 font-semibold text-stone-900">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className="w-9 h-9 rounded-full bg-stone-900 text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-2xs">
                          {emp.name.charAt(0)}
                        </div>
                        {stats.isDebt && (
                      ...
                      <div>
                        ...
                        <p className="text-xs text-stone-400 font-mono">ID: {emp.id}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-6">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-stone-500 font-medium">
                        Embauché le {new Date(emp.hireDate).toLocaleDateString('fr-FR')} ({stats.daysSinceHire} j actifs)
                      </p>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-6">
                    {stats.isDebt ? (
                      <div className="space-y-1">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-950 border border-red-300">
                        ...
                        <div className="text-2xs font-extrabold text-red-700 bg-red-50 border border-red-200 px-2.5 py-1 rounded-md flex items-center gap-1 shadow-2xs">
                          ...
                          <span>Doit régulariser <strong className="underline decoration-red-400">{stats.debtDays.toFixed(1)} jours</strong></span>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-1">
                  ...
                  <td className="py-4 px-6 text-right space-x-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 bg-stone-900 hover:bg-stone-800 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-all shadow-2xs cursor-pointer"
                    >
