...
export const LedgerHistory: React.FC<LedgerHistoryProps> = ({
  ...
  const handleExportCSV = () => {
    ...
    const rows = sortedRecords.map((rec) => {
      ...
      const remunLabel = rec.isPaid !== false ? 'Payé (Déduit du solde)' : 'Sans Solde (Non déduit)';

      return [
        escapeCSV(rec.id),
        escapeCSV(emp?.name || 'Inconnu'),
        escapeCSV(rec.employeeId),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-stone-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-0.5">
            Historique complet des congés rémunérés et sans solde, des jours de récupération et des autorisations accordées.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all shadow-xs cursor-pointer active:scale-98"
      ...
      {/* Filters Bar */}
      <div className="bg-white p-4 rounded-xl border border-stone-200/80 shadow-xs flex flex-col md:flex-row items-center justify-between gap-3">
        {/* Search */}
        <div className="relative w-full md:w-72">
          <Search className="w-4 h-4 text-stone-400 absolute left-3.5 top-3 pointer-events-none" />
          <input
            type="text"
            placeholder="Rechercher par employé ou remarque..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-9 pr-3 py-2 rounded-lg border border-stone-200 text-xs bg-stone-50 text-stone-900 focus:bg-white focus:outline-none focus:border-stone-800 transition-all"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1 text-xs">
            <span className="font-bold text-stone-400 uppercase tracking-wider">Employé:</span>
            <select
              value={selectedEmployeeId}
              onChange={(e) => setSelectedEmployeeId(e.target.value)}
              className="px-2.5 py-1.5 rounded-lg border border-stone-200 bg-stone-50 text-stone-900 text-xs focus:bg-white focus:outline-none"
            >
          ...
          {/* Leave Type Filter */}
          <div className="flex items-center gap-1 text-xs">
            <span className="font-bold text-stone-400 uppercase tracking-wider">Type:</span>
            <select
        ...
        {sortedRecords.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            ...
            <p className="text-stone-400 text-xs max-w-sm mx-auto">
              {searchTerm || selectedEmployeeId !== 'ALL' || selectedLeaveType !== 'ALL'
                ? 'Essayez de modifier vos filtres de recherche.'
                : 'Commencez par enregistrer une première demande au journal.'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-stone-50/80 border-b border-stone-200/80 text-stone-500 text-xs font-semibold uppercase tracking-wider">
                  ...
                  <th className="py-3.5 px-6">Jours Pris</th>
                      ...
                      <td className="py-4 px-6 font-semibold text-stone-900">
                        {emp ? (
                          <div>
                            ...
                            <span className="text-2xs text-stone-400">Contrat {emp.contractType === 'TYPE_A' ? 'Type A (30j/6m)' : 'Type B (30j/1an)'}</span>
                          </div>
                        ) : (
